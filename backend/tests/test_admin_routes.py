...
def admin_header():
    return {
        "authorization": f"Bearer {make_jwt({'preferred_username': 'admin@example.com', 'roles': ['Admin'], 'oid': 'admin-oid-1'})}"
    }
...
def analyst_header():
    return {
        "authorization": f"Bearer {make_jwt({'preferred_username': 'analyst@example.com', 'roles': ['Analyst'], 'oid': 'analyst-oid-1'})}"
    }
...
def test_list_users_returns_200_for_admin(client, admin_header):
    with (
        patch("routes.admin.get_connection", return_value=_mock_db_users()),
...
def test_assign_role_returns_201(client, admin_header):
    ...
    with (
        patch("routes.admin.get_connection", return_value=mock_conn),
...
def test_assign_role_returns_409_for_duplicate(client, admin_header):
    ...
    with (
        patch("routes.admin.get_connection", return_value=mock_conn),
        patch(
            "routes.admin.assign_role",
            side_effect=FastAPIHTTPException(
                status_code=409, detail="Role already assigned"
            ),
        ),
        patch("services.users_service.upsert_user"),
    ):
...
def test_remove_role_returns_204(client, admin_header):
    with (
        patch("routes.admin.remove_role") as mock_remove,
        patch("services.users_service.upsert_user"),
    ):
        resp = client.delete(
            "/admin/users/user-oid-b/roles/asgn-1", headers=admin_header
        )
