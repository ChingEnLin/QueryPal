        ...
        <>
          ...
          <aside className="fixed top-0 right-0 h-full w-full md:w-3/4 lg:w-2/3 bg-slate-900 shadow-2xl z-50 flex flex-col animate-slide-in-drawer">
            <header className="flex items-center justify-between p-4 border-b border-slate-700 flex-shrink-0">
              ...
              <button onClick={() => setIsGraphVisible(false)} className="p-1.5 rounded-full text-slate-400 hover:bg-slate-700 hover:text-white transition-colors" aria-label="Close graph view"><XIcon className="w-5 h-5" /></button>
            </header>
      ...
      const renderTableActionsToolbar = () => {
        ...
        return (
            <div id="tutorial-table-actions" className="flex items-center justify-between bg-slate-100 dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                ...
                <button onClick={() => setIsTableEditMode(!isTableEditMode)} className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${isTableEditMode ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 border border-slate-300 dark:border-slate-600'}`}>
        ...
        <div className="space-y-4 animate-fade-in">
            ...
            <div id="tutorial-view-switcher" className="flex items-center justify-between bg-slate-100 dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-1 flex-wrap">
                    ...
                    <button onClick={() => setViewMode('json')} disabled={viewMode === 'json' || isTableEditMode} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-colors disabled:cursor-not-allowed ${viewMode === 'json' ? 'bg-blue-500 text-white shadow' : 'bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 disabled:opacity-50'}`}><JsonIcon className="w-4 h-4" />JSON</button>
                    <button onClick={() => setViewMode('table')} disabled={!canBeTable || viewMode === 'table' || isTableEditMode} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-colors disabled:cursor-not-allowed ${viewMode === 'table' ? 'bg-blue-500 text-white shadow' : 'bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 disabled:opacity-50'}`}><TableIcon className="w-4 h-4" />Table</button>
                    <button onClick={() => setIsGraphVisible(true)} disabled={isTableEditMode} className={'flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-colors bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed'}><GraphIcon className="w-4 h-4" />Graph</button>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap justify-end">
                    ...
                    {viewMode === 'json' && (<button onClick={() => setIsJsonCollapsed(!isJsonCollapsed)} className="flex items-center gap-1 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-1.5 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700"><span>{isJsonCollapsed ? 'Expand' : 'Collapse'}</span><ChevronDownIcon className={`w-4 h-4 transition-transform duration-200 ${!isJsonCollapsed && 'rotate-180'}`} /></button>)}
                </div>
            </div>
